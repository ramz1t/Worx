...
class StatsPage:
    ...
    def __init__(self, username: str, reponame: str, current_user: User, request: Request) -> None:
    ...
    def make(self):
        ...
        contributors_list = get_repo_contributors(auth_params=Auth_params, repo_name=self.reponame,
                                                  users_name=db_repo.owner_username)
        commits = get_repo_commits(auth_params=Auth_params, repo_name=self.reponame, username=db_repo.owner_username)
        ...
        branches = get_repo_branches(auth_params=Auth_params, repo_name=self.reponame, username=db_repo.owner_username)
...
class NavbarData:
    ...
    def __init__(self, current_user: User, reponame: str) -> None:
        self.reponame = reponame
        self.current_user = current_user
        self.repos = []
